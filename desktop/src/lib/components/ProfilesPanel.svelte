<script lang="ts">
  ...
  import { cloneProfile, closeProfile, createProfile, deleteProfile, detectProfileDomains, openProfile, updateProfile, type ProfileInfo } from "../api";
  ...
  function engineLabel(engine: string): string { return engine === "cloak" ? "CloakBrowser" : "Playwright"; }
  ...
  async function onCreate() { const name = newName.trim().toLowerCase(); if (!/^[a-z0-9][a-z0-9-]*$/.test(name)) { panelError = "Tên profile chỉ gồm chữ thường, số và dấu -"; showToast(panelError); return; } creatingBusy = true; panelError = ""; try { await createProfile($apiKey, name, { engine: newEngine, max_tabs: newMaxTabs, headless: newHeadless }); showToast(`Đã tạo profile ${name} (${engineLabel(newEngine)})`); newName = ""; creating = false; await refreshProfiles(); } catch (e) { fail(e); } finally { creatingBusy = false; } }
  function startEdit(p: ProfileInfo) { editingId = p.id; editMaxTabs = p.max_tabs; editHeadless = p.headless === 1; editEngine = p.engine || "playwright"; editNotes = p.notes ?? ""; }
  async function saveEdit(p: ProfileInfo) { setBusy(p.id, true); panelError = ""; try { await updateProfile($apiKey, p.id, { engine: editEngine, max_tabs: editMaxTabs, headless: editHeadless, notes: editNotes }); editingId = null; await refreshProfiles(); } catch (e) { fail(e); } finally { setBusy(p.id, false); } }
  ...
  // đăng nhập mà vẫn có đường lui khi đổi engine.
  function startClone(p: ProfileInfo) { cloningId = p.id; editingId = null; cloneName = `${p.name}-copy`; cloneEngine = p.engine || "playwright"; }
  async function onClone(p: ProfileInfo) { const name = cloneName.trim().toLowerCase(); if (!/^[a-z0-9][a-z0-9-]*$/.test(name)) { panelError = "Tên profile chỉ gồm chữ thường, số và dấu -"; showToast(panelError); return; } setBusy(p.id, true); panelError = ""; try { await cloneProfile($apiKey, p.id, name, { engine: cloneEngine }); showToast(`Đã nhân bản ${p.name} → ${name} (${engineLabel(cloneEngine)})`); cloningId = null; await refreshProfiles(); } catch (e) { fail(e); } finally { setBusy(p.id, false); } }
