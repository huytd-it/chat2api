...
<Collapsible.Root bind:open={advancedOpen}>
  ...
  <Collapsible.Content>
    <div class="mt-3 grid gap-4 rounded-lg border bg-muted/20 p-3">
      ...
      <div class="flex items-center justify-between gap-4 rounded-lg border bg-card px-3 py-2 text-sm"><span><strong class="font-medium">Giữ context giữa các request</strong><span class="block text-xs text-muted-foreground">Tắt nếu site khôi phục hội thoại cũ khi mở lại tab.</span></span><Switch bind:checked={form.keepContext} aria-label="Giữ context giữa các request" /></div>
      <div class="flex items-center justify-between gap-4 rounded-lg border bg-card px-3 py-2 text-sm"><span><strong class="font-medium">Giữ cấu trúc markdown</strong><span class="block text-xs text-muted-foreground">Đọc câu trả lời theo khối (heading, list, code) thay vì text thuần.</span></span><Switch bind:checked={form.markdownFormat} aria-label="Giữ cấu trúc markdown" /></div>
      <div class="flex items-center justify-between gap-4 rounded-lg border bg-card px-3 py-2 text-sm"><span><strong class="font-medium">Lưu HTML gốc</strong><span class="block text-xs text-muted-foreground">Kèm HTML của câu trả lời vào bản ghi session để soi lại sau.</span></span><Switch bind:checked={form.captureHtml} aria-label="Lưu HTML gốc" /></div>
    </div>
  </Collapsible.Content>
</Collapsible.Root>
