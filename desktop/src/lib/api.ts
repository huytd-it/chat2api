...
export async function startPicker(key: string, profileId: number, url = ""): Promise<{ picker_id: string; profile: string; url: string }> {
  const base = await apiBase();
  const r = await fetch(base + "/admin/picker/start", { method: "POST", headers: headers(key), body: JSON.stringify({ profile_id: profileId, url }) });
  return asJson(r);
}
export async function stopPicker(key: string, pickerId: string): Promise<void> {
  const base = await apiBase();
  const r = await fetch(base + "/admin/picker/" + encodeURIComponent(pickerId) + "/stop", { method: "POST", headers: headers(key) });
  await asJson(r);
}
export async function countPickerSelector(key: string, pickerId: string, selector: string): Promise<{ selector: string; count: number; unique: boolean }> {
  const base = await apiBase();
  const r = await fetch(base + "/admin/picker/" + encodeURIComponent(pickerId) + "/count", { method: "POST", headers: headers(key), body: JSON.stringify({ selector }) });
  return asJson(r);
}
export async function capturePicker(key: string, pickerId: string): Promise<{ selector: string; best: string; locator: string; candidates: { sel: string; kind: string; unique: boolean; count: number }[]; attributes: Record<string,string>; frame?: { chain: string[]; url: string }; shadow?: { hostSelector: string | null; depth: number; mode: string | null; closed: boolean }; warning?: string }> {
  const base = await apiBase();
  const r = await fetch(base + "/admin/picker/" + encodeURIComponent(pickerId) + "/capture", { method: "POST", headers: headers(key) });
  return asJson(r);
}
