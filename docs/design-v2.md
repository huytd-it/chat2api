...
**Engine `scrapling`.** Pool giữ `AsyncStealthySession` sống cùng `BrowserContext` để recipe vẫn dùng nguyên API tab dài hạn, đồng thời nhận các patch chống fingerprint của Scrapling/Patchright. Profile dùng `user_data_dir`; chế độ `storage_state` nạp cookie và localStorage vào persistent context sau khi mở. Khi evict/drop, pool đóng session thay vì chỉ đóng context để dọn cả driver. Cần chạy `scrapling install` một lần sau khi cài package.
