...
- Produces:
  - `validate_recipe(d: dict) -> list[str]` â€” rá»—ng = há»£p lá»‡
  - `BrowserRecipe(recipe: dict, base_dir: Path, pool)` â€” `.slug`, `.url`, `.models()`, `async stream(messages, model_id)`; raise `TimeoutError` khi quÃ¡ `done_signal.timeout_ms`; stream giáº£ láº­p báº±ng poll 500ms, chá»‘ng nháº·t echo prompt.
  - Loader `_recipe_loader(directory, pool)`: Ä‘á»c `<dir>/recipe.yaml` (trá»« thÆ° má»¥c `gemini`, `openai`), Ä‘iá»n default `slug`, validate â€” lá»—i thÃ¬ in stderr + bá» qua.

- [ ] **Step 1: Fixture trang chat giáº£ láº­p**
...
Loader â€” thÃªm cuá»‘i `chat2api/router.py`:
...
(`sys` Ä‘Ã£ import á»Ÿ Ä‘áº§u `router.py`: thÃªm `import sys` náº¿u chÆ°a.)
...
- [ ] **Step 5: Cháº¡y táº¥t cáº£**

Run: `python -m pytest tests/unit tests/integration -v`
Expected: táº¥t cáº£ passed. (Test timeout dÃ¹ng selector khÃ´ng tá»“n táº¡i â†’ runner chá» tá»›i deadline.)
