...
- Modify: `chat2api/router.py` (Ä‘Äƒng kÃ½ loader)
- Test: `tests/unit/test_gemini_native.py`

**Interfaces:**
- Consumes: `Provider`, `ModelInfo` (Task 3), `flatten_messages` (Task 2).
- Produces: `GeminiNative(cfg: dict, base_dir: Path)`; helpers thuáº§n: `build_payload(prompt, model_id, think_mode) -> str`, `make_sapisidhash(sapisid) -> str`, `extract_response_text(raw) -> str` (raise `RuntimeError` khi gáº·p BardErrorInfo), `clean_text(text) -> str`. Loader `_gemini_loader(directory, pool)` Ä‘Äƒng kÃ½ vÃ o `router.LOADERS`.
...
- [ ] **Step 3: Implement** (port protocol tá»« `../gemini-web2api/gemini_web2api/gemini.py`)

`chat2api/providers/gemini_native.py`:

```python
"""Gemini web StreamGenerate protocol â€” port tá»« gemini-web2api."""
import hashlib
import json
import re
import time
...
BL = "boq_assistant-bard-web-server_20260218.00_p0"  # cáº­p nháº­t khi Gemini Ä‘á»•i "bl"
...
class GeminiNative(Provider):
    ...
    async def stream(self, messages: list[dict], model_id: str) -> AsyncIterator[str]:
        ...
        if not self._load_cookie()[0]:
            raise RuntimeError("Cookie Gemini chÆ°a cÃ³ â€” Ä‘iá»n cookie_file trong recipes/gemini/config.yaml")
...
```yaml
slug: gemini
# cookie_file trá» tá»›i JSON {"cookie": "...", "sapisid": "..."} hoáº·c chuá»—i cookie thÃ´.
# Copy cookie tá»« trÃ¬nh duyá»‡t Ä‘Ã£ Ä‘Äƒng nháº­p gemini.google.com vÃ o recipes/secrets/gemini-cookies.txt
